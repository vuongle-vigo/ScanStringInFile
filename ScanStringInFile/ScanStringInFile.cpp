...
void KMPSearch(const char* text, DWORD dataLenght, const char* pattern, LPWSTR lpPath) {
	...
	while (i < textLength) {
		...
		if (j == patternLength) {
			...
			LPWSTR pszFilename = PathFindFileNameW(lpPath); // Tên tệp tin
			wchar_t byDirectory[MAX_PATH];
			wcsncpy_s(byDirectory, lpPath, pszFilename - lpPath);
			// Column 2
			lvItem.iSubItem = 1;
			lvItem.pszText = (LPWSTR)pszFilename;
			ListView_SetItem(g_hWndListView, &lvItem);

			// Column 3
			lvItem.iSubItem = 2;
			lvItem.pszText = (LPWSTR)byDirectory;
			ListView_SetItem(g_hWndListView, &lvItem);
			//Column 4
			lvItem.iSubItem = 3;
